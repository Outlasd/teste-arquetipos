...
def comparacao_arquetipos(opcoes):
    """Compara duas opções até restar apenas uma."""
    while len(opcoes) > 1:
        opcao1, opcao2 = opcoes[:2]
        escolha = st.radio(f"Qual afirmação melhor representa você?", [arquetipos[opcao1], arquetipos[opcao2]])
...
st.title("Teste de Arquétipos")
st.write("Responda às perguntas abaixo para descobrir o seu arquétipo principal.")

# Etapa 1 - A Alma do Negócio
...
if st.button("Avançar para a escolha do arquétipo pessoal"):
    arquetipo_pessoal = comparacao_arquetipos(list(arquetipos.keys()))
    st.session_state["arquetipo_pessoal"] = arquetipo_pessoal

# Etapa 2 - O Propósito do Negócio
...
if st.button("Avançar para a escolha do arquétipo do negócio"):
    arquetipo_negocio = comparacao_arquetipos(list(arquetipos.keys()))
...
if st.button("Finalizar e ver resultado"):
    arquetipo_publico = comparacao_arquetipos(list(arquetipos.keys()))
